...
| P7 | サポートPythonバージョンがEOLを含む | 中 | pyproject.toml/setup.py |
| P8 | GFMテーブルの末尾パイプ依存 | 中   | converter.py |
| P9 | コードフェンス/引用内の表誤検出 | 中 | converter.py |
...
## P7: サポートPythonバージョンがEOLを含む

### 現状

`setup.py` の `python_requires='>=3.8'` と README の対応表記で Python 3.8 をサポート対象としているが、3.8 は 2024-10-07 に公式 EOL となった。以降の pip/CI/OS イメージでも順次サポート対象から外れていくため、継続サポートは保守コストが高い。

### 検証方法

```bash
# setup.py から python_requires を確認
grep -n "python_requires" setup.py

# README 内の対応バージョン表記を確認
rg "python3\\.8" README.md
```

### 期待される改善

1. `python_requires` を `>=3.9` 以上（もしくは LTS に合わせて 3.10+）に引き上げる
2. README / QUICKSTART / ドキュメント内の記述を最新バージョンに更新
3. CI 行列から Python 3.8 を削除し、必要なら 3.12 など最新安定版を追加
...
EOL バージョンを外すことで依存パッケージの最新機能を利用しやすくなり、セキュリティフィックスの追従もしやすくなる。
...
`parse_markdown_table()` は行を `line.split('|')[1:-1]` で切り出しているため、「行末の `|` を省略できる」という GitHub Flavored Markdown (GFM) 標準に対応できていない。末端セルが常に欠落し、Slack table ブロックの列が壊れる。
...
|---|---
| 1 | 2"""
print(parse_markdown_table(table))  # [['A'], ['1']]
```
...
1. 先頭・末尾の空要素を安全に除去しつつ、末尾 `|` の有無に関わらず全列を保持する。
2. 各行の列数が揃っているか検証し、不整合時は例外または警告を返す。
3. 設計上 `csv` モジュールや `markdown-it-py` などの既存パーサを利用してパイプエスケープ等のケースにも備える。
...
テーブル検出は `table_pattern = r'(\|.+\|[\r\n]+...)'` を `re.split()` するだけで、`` ``` `` や `~~~` のコードフェンス、`>` 引用、インラインコードを一切考慮していない。コード例や引用中の `|` がテーブルと誤判定され、Slack上で意図しない table block が挿入される。
...
print(blocks)  # tableブロックが生成されてしまう
```

### 期待される改善

1. Markdownを行ごとに走査し、コードフェンス・引用・インラインコードの状態をトラッキングしている間はテーブル検出ロジックを無効化。
2. もしくは Markdown パーサライブラリを導入して AST レベルで `table` ノードのみを処理する。
3. 追加した状態管理をカバーするエッジケーステスト（コードブロック・引用・プレーンテキスト混在）を `tests/test_converter.py` に追加。
...
echo ""
echo "[P7] Supported Python Versions"
echo "------------------------------"
PY_REQ=$(grep -o "python_requires='[^']*'" setup.py 2>/dev/null | cut -d"'" -f2)
echo "python_requires=${PY_REQ:-UNKNOWN}"
rg -n "python3\\.8" README.md >/dev/null && echo "❌ README mentions python3.8" || echo "✅ README already excludes python3.8"
