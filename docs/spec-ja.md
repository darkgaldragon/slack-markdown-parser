...
- 行単位で動くブロック機構(リッチブロック昇格・テーブル領域分割・段落内分割)は、ソフト改行をまたぐインラインコードスパンを認識しない。そのようなスパンの中にあるブロック風の行(画像・テーブル行・フェンス)は実ブロックへ昇格されうるし、予算超過の段落を分割するとスパンが分断されて対応しないバッククォートが残ることがある。内容を書き換える段(デコード・サニタイズ・URL・underscore・ゼロ幅スペース)はこれらのスパンを尊重する。ブロック風の内容を包む複数行の単一バッククォートスパンは実際にはまれで(複数行コードは通常フェンスで書かれる)、スパン対応の行消費はフォローアップとして計画している。
