...
- bare URL を Slack で安定しやすい `<https://...>` 形式にそろえる。まず URL を実際の範囲にトリミングする(GFM 風): 二重の強調記号(`**`/`~~`)・コード/山かっこ/パイプ記号(`` ` ``・`<`・`>`・`|`)・CJK 句読点(`、` `。` `」` …)で停止し、末尾の句読点(`. , ; : ! ? " ' * ~ _` と不均衡な `)`)を除外する。単独の `*`(URL のワイルドカード/クエリ)と CJK の**文字**(IRI / Unicode IDN ホスト。例 `https://ja.wikipedia.org/wiki/日本語`)は保持する。これにより、日本語のように URL の直後へ空白なしで CJK 本文が続く場合でも、URL が行末まで(閉じの `**` ごと)貪欲に飲み込んでリンク化する事故を防ぐ。
