...
- bare URL を Slack で安定しやすい `<https://...>` 形式にそろえる。まず URL を実際の範囲にトリミングする(GFM 風): 二重の強調記号(`**`/`~~`)・コード/山かっこ/パイプ記号(`` ` ``・`<`・`>`・`|`)・CJK / 全角の句読点(`、` `。` `」` `）` `！` …)で停止し、末尾の句読点(GFM の autolink 集合 `! ? . , : * _ ~` と不均衡な `)`)を除外する(`;` と引用符は URL で正当なため保持)。単独の `*`(URL のワイルドカード/クエリ)と CJK の**文字**(反復記号 `々` を含む。IRI / Unicode IDN ホスト。例 `https://ja.wikipedia.org/wiki/人々`)は保持する。これにより、日本語のように URL の直後へ空白なしで CJK 本文が続く場合でも、URL が行末まで(閉じの `**` ごと)貪欲に飲み込んでリンク化する事故を防ぐ。インラインコードスパン(段落内。掃除ルール参照)の中にある URL は素のまま残す: Slack はその区間をコードとして描画するため、`<…>` ラッパーが文字として露出してしまうからである。
...
- 意味が明確な単独 Markdown 構文を Slack ネイティブのブロックへ変換する
  ...
  - 単純な1段引用 → `rich_text_quote`
    - 引用行をまたぐインラインコードスパンを含む引用は昇格しない。単一行前提の rich_text トークナイザでは表現できず、markdown 経路ならコードとして描画されるため
  - 単純な箇条書き / 番号付きリスト → `rich_text_list`
    ...
    - 昇格したリスト項目内の Slack メンショントークンは、構造化された `rich_text` 要素へ変換する —— `<@U…>`/`<@W…>` を `user`、`<#C…>`/`<#G…>` を `channel`、`<!subteam^S…>` を `usergroup`、`<!here>`/`<!channel>`/`<!everyone>` を `broadcast` へ。`rich_text` ブロックは生トークンを解決しないため。オプションの `|label` 表示サフィックスは除去する（Slack は id から要素を描画する）
  - フェンス・引用・リストは、テキストがメッセージあたり総文字量のパッキング目標(12,800 字。「markdown ブロックのサイズ分割」参照)を超える場合は昇格しない。昇格ブロックはそのまま投稿されるため、巨大な `rich_text` が 1 つあるだけでメッセージ全体が `msg_blocks_too_long` で失敗する。この場合は生の行を `markdown` 経路に残し、任意サイズを扱える分割機構に委ねる
...
- この判定でのインラインコードスパンは、開始と同じ長さのバッククォート run でのみ閉じ、ソフト改行はまたげるが空行(段落境界)は越えない。これは実測した Slack の描画(2026-07-05)に一致する: Slack は同一段落内なら改行をまたいでバッククォートをペアリングし、その区間をインラインコードとして描画するため、中身を書き換えるとコード表示が目に見えて壊れる。したがって対になっていない孤立バッククォートが自段落の外のサニタイズを妨げることはなく、コードスパンをまたぐ無効な角括弧トークン（`<foo `bar` baz>`）はスパン内容を原文のまま保ちつつ全体を無害化する
...
- `\_escaped\_` は保持する
- bare URL、Markdown リンク、Slack の `<...>` 形式、インラインコード内の underscore は保持する（インラインコードスパンは掃除ルールと同じ段落内境界で判定する）
...
- `# 見出し |a|b|` 形式は、次の行にもパイプ（テーブル行らしい行）がある場合に限り、見出し行とテーブル行に分離する。この分離は見出し行に接着したテーブルヘッダの救済が目的であり、単にパイプを含むだけの見出し（`## Phase 1 | Overview`）はそのまま保持する（見出し内のインラインコードに含まれるパイプは無視）
- `### Heading ... Header A | Header B` のように見出しと表ヘッダが1行に潰れている場合は、次行の列形状を手がかりに first header cell を推定する。見出し末尾から参照セルと同じ語数の first cell を切り出せない場合は分離しない（`## Phase 1 | Overview` の直後にパイプ入りの散文が続いても見出しのまま保持）。分離が不成立だった見出し行はテーブルデータ行としてバッファされない。これは意図的な精度優先のトレードオフである: 先頭データセルの語数が見出し末尾から切り出せる語数を上回る接着ヘッダ（`### Report Status | Owner` + `In progress | Alice`）は、パイプ入り散文のケースと形式上区別できないため、テーブルを捏造するリスクを取らず markdown テキストのまま残す
...
- 強調マーカーは空行(段落境界)をまたいでペアリングしません（CommonMark の強調は段落を越えられない）。別々の段落にある迷子のマーカーはリテラルのまま残り、ゼロ幅スペースも挿入されません。
...
- インラインコード（`` `...` ``）は除外しない
- インラインコードスパンは段落内境界のスパンモデルに従い、ソフト改行をまたぐスパンは1トークンとして保護され、内部にゼロ幅スペースが入ることはない
...
- 行単位で動くブロック機構はソフト改行をまたぐインラインコードスパンを認識する: スパン内のブロック風の行(画像・水平線・引用・リスト・テーブル行)はリテラルの markdown テキストのまま残り、テーブル領域分割・正規化もそれらの行を対象外とし、段落内分割はスパンをまたぐ行境界をサイズ弁(パッキング目標+512字、ハードブロック制限未満に上限)の範囲で結合したまま保つ。残る制限は2つ: 弁を超える巨大スパン(またはパッキング予算を超える長さのスパン内の単一行)は依然として分断される。また、フェンスデリミタ行はスパンを終端する(CommonMark と同様、ブロック構造がインラインコードより先に確定する)ため、スパン内のフェンス風の行は実フェンスを開く。
