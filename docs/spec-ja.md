...
- bare URL を Slack で安定しやすい `<https://...>` 形式にそろえる。まず URL を実際の範囲にトリミングする(GFM 風): CJK 文字・CJK 句読点、および強調/コード/山かっこ記号(`*`・`` ` ``・`<`・`>`・`|`)で停止し、末尾の句読点(`. , ; : ! ?` と不均衡な `)`)を除外する。これにより、日本語のように URL の直後へ空白なしで CJK 本文が続く場合でも、URL が行末まで(閉じの `**` ごと)貪欲に飲み込んでリンク化する事故を防ぐ。
