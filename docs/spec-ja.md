...
3. **underscore 装飾の正規化** — Slack 互換のため `_..._` / `__...__` を `*...*` / `**...**` に寄せる。ただし snake_case 識別子や code / link / angle token / URL などの保護領域は維持
...
## underscore 装飾の正規化ルール

underscore 装飾正規化の挙動:

- `__text__` は、強調記法として成立している場合に `**text**` へ変換
- `_text_` は、強調記法として成立している場合に `*text*` へ変換
- `foo_bar_baz` のように ASCII 単語文字へ接続している underscore は変換しない
- `\_not italic\_` のようなエスケープ済み記法は変換しない
- フェンスドコード、インラインコード、Markdown リンク、angle-bracket token、bare URL の中は変換しない
